...
main (int argc, char *argv[])
{
  // This is to check the argc number
   if(argc != 2)
   {
        printf("- Invalid parameters!!!\nPlease enter the number of packets you want to send!!!  \n");
        exit(-1);
   }
  ...
  strcpy (src_ip, "172.16.85.60");

  // Destination URL or IPv4 address: you need to fill this out
  strcpy (target, "172.16.85.225");
  ...
  int number = atoi(argv[1]); // the number of packets need to send.
  int cnt = 0;
  for(; cnt<number; cnt++)
  {
    // Send packet.
