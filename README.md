...
BOOL类型，用作服务状态的标识符，initialized标识服务是否被初始化，started标识服务是否被启动，loaded标识服务是否被加载，而removable标识服务是否可以被移除

####d)  私有函数OpenDevice
OpenDevice主要进行**系统驱动设备的开启**(注意不是服务的开启)：其会通过CreateFile函数打开对应的系统设备并定义到这个设备用于操作的输入输出流（沟通用户模式和内核模式的桥梁，这里也即Hook）。其参数依次为：文件对象名(driverDosName)、访问模式、共享模式、销毁方式、创建方法、文件属性和模板文件句柄
...
####g)  InitDriver
InitDriver主要进行服务的检查和初始化。首先会检查设备是否在占用，如是则先退出设备（如果退出失败会报错），然后根据用户的输入得到driverName、driverPath和driverDosName，并且对它们进行检查，如果不符合规则就返回错误代码，否则对服务进行初始化并返回初始化成功的代码。

####h)  LoadDriver
LoadDriver主要进行服务的加载，其关键是创建一个对应的防火墙系统服务。首先看一下有没有被初始化（如果没有先要进行初始化），然后再创建一个SC_HANDLE类型的SCManager变量，并将该变量作为参数用于注册系统服务（**要将创建的服务放到该数据库中才可正常执行**）

- SCManager变量通过OpenSCManager函数进行创建，该函数的功能是**建立一个到服务控制管理器的连接**并**打开指定的数据库**。如果创建成功将返回指定的服务控制管理器数据库的句柄，错误时返回NULL。共有有三个参数：

    * 第一个参数指目标计算机名称（NULL为自己）；
    * 第二个参数为要打开的服务控制管理器数据库的名称（NULL为默认为SERVICE_ACTIVE_DATABASE）；
    * 第三个参数为权限

- SCService变量通过CreateService函数进行创建，该函数的功能是**创建一个放于指定服务控制管理器数据库的系统对象**，也即把服务加载到数据库。如果创建成功将返回服务的句柄，错误时返回NULL。一共有13个参数：

    * 第一个参数为服务控制管理器数据库的句柄；
    * 第二个参数为要安装的服务名称；
    * 第三个参数为对被用户界面来识别服务的显示名称；
    * 第四个参数为对服务的访问权限；
    * 第五个参数为服务类型（代码中的SERVICE_KERNEL_DRIVER说明为驱动服务程序）；
    * 第六个参数为服务启动选项（代码中的SERVICE_DEMAND_START说明为服务控制管理器启动）；
    * 第七个参数为当该服务启动失败时产生错误的严重程度及保护措施（代码中的SERVICE_ERROR_NORMAL说明将服务记录到日志中并继续执行）；
    * 第八个参数为该服务程序的路径；
    * 第九个参数为该服务程序所属的注册表组的名字（NULL指不属于任何组）；
    * 第十个参数与第九个参数配合使用，说明该服务在第九个参数中指定的组中的标记值；
    * 第十一个参数说明服务在该组中的执行先决条件；
    * 第十二个参数为可执行该服务的账户名（NULL说明本地账户）；
    * 第十三个参数为执行该服务的密码

####i)  UnloadDriver
UnloadDriver主要进行服务的注销，其会对现有的服务进行依次的检查。一般来说会经历初始化->加载->启动三步，因此在这里先要弄清楚服务启动了没有，如果有就要先关闭服务（StopDriver函数）。然后再直接通过函数DeleteService注销服务。

####j)  StartDriver
StartDriver主要进行服务的启动(前面加载好了，还得启动才能正式起作用），主要通过系统提供的StartService函数进行。在这个函数中需要传入三个参数:

- 第一个参数要传入服务对应的指针，该指针可通过OpenService函数得到；
- 第二个参数和第三个参数为启动该服务的参数（argc和argv）。然后调用OpenDevice函数打开对应的系统设备用于和用户进行交互（会返回对应设备操作的句柄）。
- **Notice：** 在OpenService函数中需要传入服务所在服务控制管理器数据库的名称，要启动的服务名和访问类型。

####k)  StopDriver
StopDriver主要进行服务的关闭，主要通过系统提供的ControlService函数进行（因为这里只是将服务停掉，没有将服务完全干掉）。在这个函数中需要传入三个参数：

- 第一个参数要传入服务对应的指针，该指针可通过OpenService函数得到；
- 第二个参数为要对对应服务做的操作(这里是关闭)；
- 第三个必须传入一个参数为系统服务状态的结构体（其实这个也是函数返回值，但是由于返回值只能有一个，因此只能用这种办法）。
